...
float ExpressionTokens::PerformOperation(float value1, float value2, std::string symbol) {
    switch (symbol[0]) {
        case '+': return Addition(value1, value2).Evaluate();
        case '*': return Multiplication(value1, value2).Evaluate();
        case '-': return Subtraction(value1, value2).Evaluate();
        case '/': return Division(value1, value2).Evaluate();
        case '%': return Modulus(value1, value2).Evaluate();
        case '^': return Exponentiation(value1, value2).Evaluate();
        case 'O': return LogicalOR(value1, value2).Evaluate();
        case 'A': return LogicalAND(value1, value2).Evaluate();
    }
    return 0;
}